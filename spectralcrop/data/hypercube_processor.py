...
class HypercubeProcessor:
    ...
    def __init__(
        ...
        img,
        cube,
        wavelengths: Sequence | None = None,
        fwhm: Sequence | None = None,
        scale: float = 10000.0,
    ...
    @staticmethod
    def _is_in_windows(value: float, windows: tuple[tuple[float, float], ...]) -> bool:
    ...
    def build_valid_bands(
        self,
        wavelengths: np.ndarray | None = None,
        water_windows: tuple[tuple[float, float], ...] | None = None,
        ...
        windows = (
            self.water_windows
            if water_windows is None
            else tuple(tuple(map(float, w)) for w in water_windows)
        )
    ...
    def to_reflectance(
        ...
        ignore_vals: Sequence | None = None,
    ...
    def _write_reflectance_blockwise_bands(
        self,
        arr_reflect,
        valid_bands,
        water_windows=None,
        ...
        _H, _W = self.img.nrows, self.img.ncols
        ...
        if progress is not None:
            iterator = progress(
                iterator,
                total=(len(work_bands) + band_block - 1) // band_block,
                desc="Batches(bands)",
            )
    ...
    def _write_reflectance_chunkwise_tiled(
        self,
        arr_reflect,
        valid_bands,
        water_windows=None,
        ...
        if not c_chunk:
            raise ValueError(
                "No se pudo determinar c_chunk; asegúrate de crear el Zarr con chunks=(C,Y,X)."
            )
        ...
        tiles_y = (H + Ty - 1) // Ty
        tiles_x = (W + Tx - 1) // Tx
        _total_steps = tiles_y * tiles_x * ((B_out + c_chunk - 1) // c_chunk)
        ...
        for y0 in range(0, H, Ty):
            ...
            for x0 in range(0, W, Tx):
                ...
                for c0 in range(0, B_out, c_chunk):
                    ...
                    raw = self._read_bands_block(
                        group_bands, yslice=ysl, xslice=xsl
                    )  # (c_len, Ty', Tx')
                    ...
                    if wl is not None:
                        ...
                        water_flags = np.array([in_water(b) for b in group_bands], dtype=bool)[
                            :, None, None
                        ]
                    else:
    ...
    def save_reflectance_to_zarr_fast(
        self,
        zarr_path: str,
        exclude_water: bool = True,
        ...
        def _set_dim_attrs(zarr_array, dims):
            try:
                ...
                zarr_array.attrs.update(
                    {"dimension_names": list(dims), "_ARRAY_DIMENSIONS": list(dims)}
                )

        # Validaciones básicas
        H, W, Bm = self.cube.shape
        assert (H == self.img.nrows) and (W == self.img.ncols), (
            "Dimensiones memmap vs img no coinciden."
        )
        ...
        if "reflectance" in root:
            ...
            if arr_reflect.shape != (B_out, H, W):
                raise ValueError(
                    f"'reflectance' existente con shape {arr_reflect.shape}, esperado {(B_out, H, W)}"
                )
            ...
            try:
                ...
                arr_reflect = root.create_array(
                    name="reflectance", shape=(B_out, H, W), chunks=chunks, dtype="float32"
                )
                _set_dim_attrs(arr_reflect, ["band", "y", "x"])
        ...
        if resume:
            if "written_bands" not in root:
                try:
                    wb = root.create_array(
                        "written_bands",
                        shape=(B_out,),
                        chunks=(min(B_out, 1024),),
                        dtype="uint8",
                    ...
                    wb = root.create_array(
                        "written_bands", shape=(B_out,), chunks=(min(B_out, 1024),), dtype="uint8"
                    )
                    wb[:] = 0
                    _set_dim_attrs(wb, ["band"])
            else:
            ...
            root.attrs.put(
                {
                    ...
                    "water_windows_nm": tuple(
                        tuple(map(float, w))
                        for w in (self.water_windows if water_windows is None else water_windows)
                    ),
                }
            )
            # Coordenadas/índices
            try:
                arr_band = root.create_array(
                    "band",
                    shape=(B_out,),
                    chunks=(min(B_out, 1024),),
                    dtype="int32",
                ...
                arr_band = root.create_array(
                    "band", shape=(B_out,), chunks=(min(B_out, 1024),), dtype="int32"
                )
                arr_band[:] = np.arange(B_out, dtype=np.int32)
                _set_dim_attrs(arr_band, ["band"])
            ...
            if add_coords and (self.wavelengths is not None):
                ...
                try:
                    arr_wl = root.create_array(
                        "wavelength",
                        shape=wl_valid.shape,
                        chunks=(wl_valid.shape[0],),
                        dtype="float32",
                        dimension_names=["band"],
                    )
                    arr_wl[:] = wl_valid
                ...
                if (self.fwhm is not None) and (len(self.fwhm) == len(self.wavelengths)):
                    ...
                    try:
                        arr_fw = root.create_array(
                            "fwhm",
                            shape=fw_valid.shape,
                            chunks=(fw_valid.shape[0],),
                            dtype="float32",
                            dimension_names=["band"],
                        )
                        arr_fw[:] = fw_valid
        ...
        print(
            f"[INFO] Estrategia: {strategy} | band_block={band_block} | tile={tile if strategy == 'tiled' else 'N/A'}"
        )

        if tqdm is not None:
            if strategy == "bands":
                pbar = tqdm(
                    total=(len([b for b in pending_bands]) + band_block - 1) // band_block,
                    desc="Batches(bands)",
                )
        ...
        if add_ndvi and ("wavelength" in root):
            ...
            try:
                ...
                try:
                    ...
                    arr_ndvi = root.create_array(
                        "NDVI", shape=(H, W), chunks=yx_chunks, dtype="float32"
                    )
                    arr_ndvi[:] = NDVI.astype("float32")
                    _set_dim_attrs(arr_ndvi, ["y", "x"])
                ...
                try:
                    ...
                    vm = root.create_array(
                        "veg_mask", shape=(H, W), chunks=yx_chunks, dtype="uint8"
                    )
                    vm[:] = veg_mask.astype("uint8")
                    vm.attrs.update({"0": "no-veg", "1": "veg"})
                    _set_dim_attrs(vm, ["y", "x"])
        ...
        if self.wavelengths is not None:
            ...
            print(
                f"    Rango λ exportado: {float(np.nanmin(wl_exp)):.1f}–{float(np.nanmax(wl_exp)):.1f} nm"
            )
    ...
    def compute_ndvi(self, red_nm: float = 660.0, nir_nm: float = 800.0, threshold: float = 0.3):
        """
